...
近几年来，又涌现出相当多优秀的 Agent。其中，OpenCode 是我最先并且到现在一直使用的 Agent 工具。开源，兼容，生态好，OpenCode 的有点自然不必多说。

但在 IDEA Terminal 中直接使用，发现了很多问题，包括但不限于，鼠标滚轮无法准确滚动聊天记录，右侧滑块会随鼠标跳转，快捷键互相影响（有的时候想复制直接就关闭进程了）...
另外我还想添加一些功能，例如 OpenCode 可以 @ 引入文件，但无法通过代码快捷在 terminal 中生成，用起来诸多不变。
因此我萌生了用 AI 重写一个 IDEA 插件的想法。其实在写之前，也想看看有没有现成的插件，有一个关于 OpenCode 插件，但我感觉使用体验和我想的不一样，于是决定自己重写。

之前一直自己用，作用也就是套壳一下 OpenCode，添加点辅助功能，我觉得也没必要发出来。但前几天基于 OpenCode 开发的 Mimo Code 也发布了。我觉得我可以把我习惯用的 TUI 工具都集成进来，这样这个工具的用处就大了很多。
目前可以使用OpenCode，MimoCode 和 Reasonix （1.x 版本可以直接启动，0.x版本可以Custom 输入 npx reasonix code，这是以前官网推荐的启动方式）

其实我原本是不想开源出来的，并不是我敝帚自珍。只是第一这代码并不是我写的，都是 AI 生成的，我只是回车执行，我自己也不懂 Kotlin，只能用 Java 的一些思路来规划，对代码的审核也不严谨。
而且大家现在也不会看代码学习，开源出来其实意义不大，但发布到市场中需要填写一个地址，因此我还是开源以后使用github地址好一点。

总之，希望能对大家有帮助，也算是一个小小 vibecoding 的作品。所有代码均由 deepseek v4pro 和 mimo v2.5pro 生成，可能用了一次GLM 5.1，国产模型其实很能打。毕竟开发这个 IDEA 插件，资料肯定是比 Spring 少，遇到的问题能解决实属不易。
希望国产模型厂商能继续加油，咬死那些封闭模型，让我们早日实现token自由，哈哈 (●'◡'●)
